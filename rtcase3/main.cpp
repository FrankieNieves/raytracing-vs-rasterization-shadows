...
void setupCase1(Scene& scene) {
    ...
    // Spheres
    scene.spheres.clear();
    scene.spheres.push_back(Sphere(Vec3f(-1.5f, 0, 3.0f), 0.5f, sphereMat1));
    scene.spheres.push_back(Sphere(Vec3f(1.5f, 0, 3.0f), 0.5f, sphereMat2));
    scene.spheres.push_back(Sphere(Vec3f(0.0f, 0, 2.0f), 0.5f, sphereMat3));
    scene.spheres.push_back(Sphere(Vec3f(-1.5f, 0, 1.0f), 0.5f, sphereMat4));
    scene.spheres.push_back(Sphere(Vec3f(1.5f, 0, 1.0f), 0.5f, sphereMat5));

    // Lights: keep original + add second light
    scene.lights.clear();
    scene.lights.push_back(Light(Vec3f(3.0f, 0.8f, 3.0f), Color(200,200,200), 0.8f));  // original light
    scene.lights.push_back(Light(Vec3f(-3.0f, 0.8f, 3.0f), Color(200,200,200), 0.8f)); // new light
...
struct ShadowMetrics {
    int shadowPixels = 0;
    float shadowAreaRatio = 0.0f;
    int diffPixels = 0; // if comparing two images
    float diffRatio = 0.0f;
};

// Count shadow pixels from shadow mask image
ShadowMetrics ComputeShadowMetrics(const Image& shadowMask, const Image* baseline = nullptr) {
    ShadowMetrics metrics;
    int totalPixels = shadowMask.W * shadowMask.H;

    for (int i = 0; i < totalPixels; ++i) {
        const auto& p = shadowMask.pix[i];
        if (p.brightness() < 0.3f) metrics.shadowPixels++;

        // Pixel-wise shadow difference
        if (baseline) {
            const auto& bp = baseline->pix[i];
            bool shadow1 = p.brightness() < 0.3f;
            bool shadow2 = bp.brightness() < 0.3f;
            if (shadow1 != shadow2) metrics.diffPixels++;
        }
    }

    metrics.shadowAreaRatio = float(metrics.shadowPixels) / totalPixels;
    if (baseline) metrics.diffRatio = float(metrics.diffPixels) / totalPixels;
    return metrics;
}
...
int main() {
    ...
    Image image(W, H);
    Image shadowMask(W, H, Color(255,255,255));
    ...
    for (int y = 0; y < H; ++y) {
        for (int x = 0; x < W; ++x) {
            ...
            Ray r(cameraPos, normalize(forward + right * px + cameraUp * py));
            ...
            if (scene.intersect(r, hr)) {
                ...
                for (auto& light : scene.lights) {
                    if (scene.isInShadow(hr.point, light.position)) {
                        inShadow = true;
                        break;
                    }
                }
                shadowMask.PutPixel(x, y, inShadow ? Color(0,0,0) : Color(255,255,255));
            } else {
    ...
    image.SavePPM("rtcase3.ppm");
    shadowMask.SavePPM("rtcase3_shadowmask.ppm");

        ShadowMetrics metrics = ComputeShadowMetrics(shadowMask);

        cout << "\n=== SHADOW METRICS ===\n";
        cout << "Shadow pixels (brightness < 0.3): " << metrics.shadowPixels << "\n";
        cout << "Shadow area ratio: " << (metrics.shadowAreaRatio * 100.0f) << " %\n";
        cout << "Render time: " << renderMs << " ms\n";
