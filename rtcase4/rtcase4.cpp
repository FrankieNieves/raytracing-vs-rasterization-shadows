...
void setupCase2(Scene& scene) {
    // Material
    Material sphereMat(Color(200, 200, 255), 0.2f, 0.8f, 0.3f, 32.0f);

    // Clear previous objects
    scene.spheres.clear();
    scene.lights.clear();

    // Sphere at origin (slightly above ground)
    scene.spheres.push_back(Sphere(Vec3f(0.0f, 0.0f, 3.0f), 0.5f, sphereMat));

    // Two lights from opposite sides
    scene.lights.push_back(Light(Vec3f(-3.0f, 1.0f, 3.0f), Color(255, 255, 255), 1.0f)); // left
    scene.lights.push_back(Light(Vec3f(3.0f, 1.0f, 3.0f), Color(255, 255, 255), 1.0f));  // right
...
int main() {
    ...
    image.SavePPM("rtcase4.ppm");
    shadowMask.SavePPM("rtcase4_shadowmask.ppm");
